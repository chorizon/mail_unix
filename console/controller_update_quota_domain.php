...
function update_quota_domainConsole() 
{

    //Make standard class method with this.

    Task::make_simple_petition_ssh(['category' => 'mail', 'module' => 'mail_unix', 'script' => 'update_quota_domain.py'], 'callback');
...
function callback($arr_args, $arr_extra_args)
{
    ...
    if($arr_extra_args['domain_id']>0)
    {
        ...
        Webmodel::$model['mail_server_unix']->reset_require();
    
        Webmodel::$model['mail_server_unix']->set_conditions(['WHERE IdMail_server_unix=?', [$arr_extra_args['domain_id']]]);
    
        Webmodel::$model['mail_server_unix']->update(array('quota' => $arr_args['quota'], 'num_accounts' => $arr_extra_args['num_accounts']));
